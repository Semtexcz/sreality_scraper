...
`TASK-061`, leaving the resumable scraper recovery work as the remaining
scraper-reliability follow-up ahead of the notebook-analysis batch.

| Order | Task | Focus | Phase | Why now |
| --- | --- | --- | --- | --- |
| 1 | `TASK-062` | Resumable scrape mode | Scraper recovery workflow | Now that nationwide traversal no longer stops on a single stale duplicate window, operators still need a supported way to continue interrupted crawls without re-downloading already persisted detail pages from the start. |
