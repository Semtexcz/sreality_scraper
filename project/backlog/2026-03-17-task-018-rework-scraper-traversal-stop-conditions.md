...
- [ ] Revisit the current page traversal algorithm and document the supported stop
      conditions for region scraping.
- [ ] Replace the current `max(strana) + 1` pagination assumption with traversal
      logic that can stop based on observed listing-page outcomes rather than a
      single HTML heuristic.
- [ ] Define explicit handling for empty listing pages, repeated estate URL sets,
      and other signals that indicate traversal exhaustion or upstream drift.
- [ ] Ensure region traversal remains bounded by operator-supplied `max_pages` and
      `max_estates` limits.
- [ ] Add deterministic unit tests for multi-page traversal, early-stop behavior,
      duplicate-page detection, and pagination drift cases.
- [ ] Update operator-facing documentation if runtime semantics for page traversal
      change.
