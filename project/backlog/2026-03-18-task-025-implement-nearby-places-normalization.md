...
- [ ] Extend normalization models and runtime to emit typed nearby-place
      entries for all supported source categories.
- [ ] Parse source values such as `ZŠ Kouřim(352 m)` into explicit `name` and
      integer `distance_m` fields while preserving the original `source_text`.
- [ ] Keep malformed or unsupported nearby-place values traceable without
      failing normalization for the whole listing.
- [ ] Add focused unit tests and at least one regression fixture derived from
      persisted snapshots.
- [ ] Update normalization contract documentation and any version constants that
